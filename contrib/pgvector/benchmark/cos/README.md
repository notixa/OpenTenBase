...
针对 **Cosine（余弦）** 距离的 IVFFlat 基线。本目录自包含，可独立运行完整流程。
...
| 表名 | `glove_base` | `lastfm_base` | `glove_base` |
| 运算符 | `<=>` | `<#>` | `<=>` |
| opclass | `vector_cosine_ops` | `vector_ip_ops` | `vector_cosine_ops` |
| ground truth 距离 | Cosine（余弦） | 内积（dot） | 余弦（angular） |
